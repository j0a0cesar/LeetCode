...
const UserStat = () => {
  ...
  const [stats, setStats] = useState({
    ...
    aceitos: 0,
    rejeitados: 0,
    taxaAceitacao: 0, // number!
  });

  const { authUser, setAuthUser } = useAuthContext();
  const navigate = useNavigate();

  const authHeader = authUser?.token
  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      ...
      try {
        ...
        const taxaAceitacao = total > 0 ? (aceitos / total) * 100 : 0; // número
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(maybeText || 'Falha ao apagar a conta');
      }
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              {/* “Olá”/“Bem-vindo” agora deve aparecer, pois o AuthContext foi corrigido */}
              Bem-vindo,{' '}
              <span className="font-semibold text-green-600">
                {authUser?.nomeUsuario}
              </span>
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {/* Ícone “trash” simples em SVG */}
              <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                <path d="M6 8a1 1 0 012 0v6a1 1 0 11-2 0V8zm4 0a1 1 0 012 0v6a1 1 0 11-2 0V8z" />
                <path
                  fillRule="evenodd"
                  d="M4 5a1 1 0 011-1h2l1-1h4l1 1h2a1 1 0 011 1v1H4V5zm2 3h8l-.8 8.4A2 2 0 0111.2 18H8.8a2 2 0 01-1.99-1.6L6 8z"
                  clipRule="evenodd"
                />
              </svg>
              Apagar conta
                ...
                <tbody>
                  {submissions.map((sub) => (
                    <tr key={sub.id}>
                      ...
                      <td className="font-medium">{sub.problemaTitulo}</td>
                      <td>
                        <span
